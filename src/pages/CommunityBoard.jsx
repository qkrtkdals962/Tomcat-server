...
const FreeBoard = () => {
  const loggedInUsername = "익명"; // 실제 로그인 사용자 정보로 대체할 수 있음

  const [posts, setPosts] = useState(dummyData); // 게시글 목록 (임시 데이터 사용)
  const [newPost, setNewPost] = useState({ title: "", content: "", tags: "", authorName: loggedInUsername });
  ...
  const handleCreatePost = async () => {
    ...
    console.log("보낼 JSON", JSON.stringify(postToSend, null, 2));
      ...
      <ul className="post-list">
        {currentPosts.map((post) => (
          <li key={post.id} className="post-item">
            <h3 onClick={() => handleShowPost(post)} className="post-title">
              {post.title}
            </h3>
            ...
            <div className="post-actions">
              ...
              <button
                ...
                onClick={() => handleReport("게시글", post.id)}
              >
                신고
              </button>
              {isLoggedIn && (
      ...
      {selectedPost && (
        <div className="post-detail">
          ...
          <p className="detail-meta">
            작성자: {selectedPost.author} | 작성일:{" "}
            {new Date(selectedPost.createdAt).toLocaleDateString()} | 조회수:{" "}
            {selectedPost.views}
          </p>
              ...
              {(comments[selectedPost.id] || []).map((comment) => (
                <li key={comment.id} className="comment-item">
                  ...
                  <div className="comment-actions">
                    ...
                    <button
                      onClick={() =>
                        handleCommentDislike(selectedPost.id, comment.id)
                      }
                    >
                      👎 {comment.dislikes}
                    </button>
                      ...
                      .map((reply) => (
                        <li key={reply.id} className="comment-item reply-item">
                          ...
                          <div className="comment-actions">
                            <button
                              onClick={() =>
                                handleCommentLike(selectedPost.id, reply.id)
                              }
                            >
                              👍 {reply.likes}
                            </button>
                            <button
                              onClick={() =>
                                handleCommentDislike(selectedPost.id, reply.id)
                              }
                            >
                              👎 {reply.dislikes}
                            </button>
